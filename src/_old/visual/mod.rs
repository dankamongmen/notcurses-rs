...
use crate::{
    ncresult,
    sys::{NcVisual, NcVisualOptions, NcVisualFlag},
...
//     'ncvisual: 'ncplane,
{
    ...
    pub fn render_plane(&mut self, nc: &mut Notcurses) -> NResult<()> {
        ...
        self.options.flags &= !NcVisualFlag::ChildPlane as u64;
        // TODO:FIXME: blit
        #[allow(deprecated)]
...
impl<'ncvisual, 'ncplane> Visual<'ncvisual> {
    ...
    pub fn set_interpolate(&mut self, interpolate: bool) {
        if interpolate {
            self.options.flags &= !NcVisualFlag::NoInterpolate as u64;
        } else {
            self.options.flags |= NcVisualFlag::NoInterpolate as u64;
        }
    }
    ...
    pub fn set_transparent_color(mut self, color: Option<u32>) -> Self {
        if let Some(color) = color {
            self.options.flags |= NcVisualFlag::AddAlpha as u64;
